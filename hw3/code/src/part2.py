...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            # if detect multiple ids, select ids=0
            if(len(ids) > 1):
                corners = corners[0]

            # TODO: 2.find homograpy
            # function call to solve_homography()
            corners_reshaped = (np.array(corners, dtype='int')).reshape((4,2))
            H = solve_homography(ref_corns, corners_reshaped)

            # TODO: 3.apply backward warp
            # function call to warping()
            xmin, ymin = np.min(corners_reshaped, axis=0)
            xmax, ymax = np.max(corners_reshaped, axis=0)
