...
def get_tiny_images(img_paths):
    ...
    for path in img_paths:
        img = Image.open(path) 
        resize_img = np.array(img.resize((16,16)).convert('L'))

        img_min, img_max = np.min(resize_img), np.max(resize_img)
        norm_img = (resize_img - img_min)/(img_max - img_min)
        
        norm_img = norm_img.flatten()
        tiny_img_feats.append(norm_img)
...
def build_vocabulary(img_paths, vocab_size=400):
    ...
    # Parameters for dsift, can modify
    step_size = 5 
    is_fast = True 

    for path in img_paths:
        print("curr processing image: "+str(path))
        img = Image.open(path) 
        gray_img = np.array(img.convert('L'))

        _keypoint, descriptor = dsift(gray_img, step=[step_size,step_size], fast=is_fast) 
        if descriptor is not None:
            for d in descriptor:
                bag_of_features.append(d)
        
        img.close()

    vocab = kmeans(np.array(bag_of_features).astype('float32'),vocab_size, initialization="PLUSPLUS")

    print("vocab processing finished")
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for path in img_paths:
        ...
        hist, bin_edges = np.histogram(closest_cluster, bins=len(vocab))
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    # get k-nearest training features for each images, can modify
    k = 8
    dist = cdist(test_img_feats, train_img_feats, 'minkowski', p=0.5)
    k_nearest_feat = np.argsort(dist, axis=1)[:,:k]

    for each_img_feat in k_nearest_feat:
        # convert label id to string label names
        nearest_labels = [train_labels[i] for i in each_img_feat]
        
        # vote the most common label as our final predict for the image
        vote = max(set(nearest_labels), key=nearest_labels.count)
        test_predicts.append(vote)
