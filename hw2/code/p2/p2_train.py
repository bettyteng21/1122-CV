...
# def get_pseudo_label_weight(epoch, max_weight=1.0, rampup_epochs=100, ceiling=0.25):
#     # For semi-supervised learning
#     # Linearly ramp up the weight over rampup_epochs
#     if epoch < rampup_epochs:
#         return min(max_weight * (epoch / rampup_epochs), ceiling)
#     else:
#         return min(max_weight, ceiling)


def train(model, train_loader, unlabel_loader, val_loader, logfile_dir, model_save_dir, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(cfg.epochs):
        ...
        # ##### SSL #####
        # ######################### TODO Begin ##########################
        # unlabel_start_time = time.time()
        # unlabel_loss = 0.0
        # unlabel_correct = 0.0
        # model.eval()

        # pseudo_label_weight = get_pseudo_label_weight(epoch=epoch,rampup_epochs=cfg.epochs*2, ceiling=0.1)

        # for batch, data in enumerate(unlabel_loader):
        #     sys.stdout.write(f'\r[{epoch + 1}/{cfg.epochs}] Unlabel batch: {batch + 1} / {len(unlabel_loader)}')
        #     sys.stdout.flush()
        ...
        #     # Forward pass. input: (batch_size, 3, 32, 32), output: (batch_size, 10)
        #     pred = model(images)
        #     confidence, pred_labels = torch.max(pred, 1)

        #     mask = confidence > cfg.confidence_threshold
        #     confident_images = images[mask]
        #     confident_pred_labels = pred_labels[mask]

        #     # skip this data if it's pred label is not clear enough
        #     if len(mask) == 0:
        #         continue

        #     model.train()
            
        #     outputs = model(confident_images)
        #     pseudo_loss = criterion(outputs, confident_pred_labels) * pseudo_label_weight
        ...
        #     # Evaluate.
        #     unlabel_correct += torch.sum(torch.argmax(outputs, dim=1) == confident_pred_labels)
        #     unlabel_loss += pseudo_loss.item()
        # ######################### TODO End ##########################

        # # Print validation result
        # unlabel_time = time.time() - unlabel_start_time
        # unlabel_acc = unlabel_correct / len(unlabel_loader.dataset)
        # unlabel_loss /= len(unlabel_loader)
        # print()
        # print(f'[{epoch + 1}/{cfg.epochs}] {unlabel_time:.2f} sec(s) Unlabel Acc: {unlabel_acc:.5f} | Unlabel Loss: {unlabel_loss:.5f}')
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(val_loader):
                sys.stdout.write(f'\r[{epoch + 1}/{cfg.epochs}] Validating...')
                sys.stdout.flush()
                # Data loading.
                images, labels = data['images'].to(device), data['labels'].to(device) # (batch_size, 3, 32, 32), (batch_size)
                # Forward pass. input: (batch_size, 3, 32, 32), output: (batch_size, 10)
